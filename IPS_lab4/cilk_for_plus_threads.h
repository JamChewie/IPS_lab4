#ifndef CILKPLUSTHREADS_H
#define CILKPLUSTHREADS_H
#include <iostream>
#include <vector>
#include <thread>
#include <stdio.h>
...
namespace CILK_PLUS_THREADS
{
	...
	void FindAverageValues(eprocess_type proc_type, double** matrix, const size_t numb_rows, const size_t numb_cols, double* average_vals)
	{
		...
		case eprocess_type::by_rows:
		{
			for (size_t i = 0; i < numb_rows; ++i)
			{
				//—оздаем reducer_opadd чтобы избежать гонок данных в cilk_for
